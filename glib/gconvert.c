 ...
 * @str: (array length=len) (element-type guint8): the string to convert
 ...
 *                 input sequence.
 * @bytes_written: (out): the number of bytes stored in the output buffer
 ...
 * @to_codeset:   name of character set into which to convert @str
 * @from_codeset: character set of @str.
 * @fallback: (nullable): UTF-8 string to use in place of character not
 ...
 *                 stored will the byte offset after the last valid
 *                 input sequence.
 * @bytes_written: (out) (optional): the number of bytes stored in the
 *                 output buffer (not including the terminating nul).
 * @error:         location to store the error occurring, or %NULL to ignore
 *                 errors. Any of the errors in #GConvertError may occur.
 * 
...
/**
 * g_filename_to_utf8:
 * @opsysstring: (array length=len) (element-type guint8): a string
 *                 in the encoding for filenames.
 * @len:           the length of the string, or -1 if the string is
 *                 nul-terminated (Note that some encodings may allow nul
 *                 bytes to occur inside strings. In that case, using -1
 *                 for the @len parameter is unsafe)
...
/**
 * g_filename_to_uri:
 * @filename: (type filename): an absolute filename specified in the GLib
 *            file name encoding, which is the on-disk file name bytes on
 *            Unix, and UTF-8 on Windows.
 ...
 * @filename: (type filename): an absolute pathname in the GLib file name encoding
 *
 ...
 * @filename: (type filename): a pathname hopefully in the GLib file name encoding
 * 
